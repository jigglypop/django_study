...
class PostViewset(viewsets.ModelViewSet):
    ...
    #                 ContentFile(imgdata, name=file_url),
    #                 "languagetogetherimage",
    #                 file_url,
    ...
    def list(self,request,*args,**kwargs):
        if request.GET.get('nickname'):
            ...
            queryset = list(Profile.objects.filter(
                nickname__icontains=nickname).values('user','user_image','nickname'))[:5]
            return JsonResponse(queryset,safe=False)
        else:
            return JsonResponse({})
...
class PostLikeViewset(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        post = get_object_or_404(queryset, pk=pk)
        if post:
            postprofile = list(post.profiles.values('user','username','nickname','user_image',))
...
class EventJoinViewset(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        if event:
            eventprofile = list(event.profile.values('user','username','nickname','user_image',))
            page = request.GET.get('page')
            if page:
                # 여기 페이지 수 조절할것
...
class FollowerViewset(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        if follower:
            followerprofile = list(follower.follower.values('user','username','nickname','user_image',))
            page = request.GET.get('page')
            if page:
                # 여기 페이지 수 조절할것
...
class FollowingViewset(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        if following:
            followingprofile = list(following.following.values('user','username','nickname','user_image',))
            page = request.GET.get('page')
            if page:
                # 여기 페이지 수 조절할것
